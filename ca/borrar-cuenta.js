...
const motivoDelError = async (error) => {
  ...
  return 'No hem pogut esborrar el compte. Escriu-nos a soporte@micarga.es i ho fem nosaltres.';
};
...
$('form-correo').addEventListener('submit', async (e) => {
  ...
  await ocupado($('btn-correo'), 'Enviando…', async () => {
    ...
    if (error) {
      ...
      avisar(noExiste
        ? 'No hi ha cap compte de Mi Carga amb aquest correu. Comprova que és el que feies servir, o escriu-nos a soporte@micarga.es.'
        : 'No t\'hem pogut enviar el codi. Torna-ho a provar d\'aquí a un minut.');
      return;
    }
    correoEnCurso = correo;
    $('ayuda-codigo').textContent =
      `T'hem escrit a ${correo}. Copia aquí el codi de 6 dígits; si el ` +
      `correu porta un enllaç, prement-lo també serveix. Si no el veus, mira a la carpeta de correu brossa.`;
...
$('form-codigo').addEventListener('submit', async (e) => {
  ...
  await ocupado($('btn-codigo'), 'Comprobando…', async () => {
    ...
    if (error) { avisar('El codi no és correcte o ha caducat. Demana\'n un de nou.'); return; }
