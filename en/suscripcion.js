...
const irAPasoCodigo = (email) => {
  ...
  $('ayuda-codigo').textContent =
    `We've written to ${email}. Copy the code in here; if the ` +
    `email has a link, tapping it gets you in too. If you can't see it, check your spam folder.`;
...
$('form-alta').addEventListener('submit', async (e) => {
  ...
  if (!f.elements['acepto'].checked) {
    avisar('To create the account you need to accept the terms and the privacy policy.');
    return;
  }
  if (val('password').length < 6) {
    avisar('The password needs at least 6 characters.');
    return;
  }
  if (!nifValido(val('nif_cif'))) {
    avisar('That tax ID isn\'t valid. Check that the letter matches the numbers.');
  ...
  if (!telefono) {
    avisar('That phone number isn\'t valid. Enter a 9-digit Spanish mobile number starting with 6 or 7.');
  ...
  await ocupado($('btn-alta'), 'Creando…', async () => {
    ...
    if (error) {
      ...
      avisar(duplicado
        ? 'We couldn\'t create the account. The likeliest reason is that this phone number is already registered with another email address. Try a different number or write to us at soporte@micarga.es.'
        : 'We couldn\'t create the account. Try again in a minute.');
      return;
    }
...
$('form-contrasena').addEventListener('submit', async (e) => {
  ...
  await ocupado($('btn-contrasena'), 'Entrando…', async () => {
    ...
    if (error) {
      avisar('That password isn\'t right. Try again or use the code we\'ve emailed you.');
      return;
    }
    datosAlta = null;
    await pedirEnlaces();
...
$('form-codigo').addEventListener('submit', async (e) => {
  ...
  if (token.length < 6 || token.length > 10) {
    avisar('Copy the whole code, exactly as it appears in the email.');
    return;
  }
  ...
  await ocupado($('btn-codigo'), 'Comprobando…', async () => {
    ...
    if (error) {
      avisar('That code is wrong or has expired. Ask for a new one.');
      return;
    }
    ...
    if (datosAlta?.password) {
      ...
      if (errPass) {
        avisar('Your account has been created, but we couldn\'t save the password. You can set it from the app using “Forgotten your password?”. Let\'s carry on with the payment.', 'info');
      }
      datosAlta = null;
    }
...
$('form-facturacion').addEventListener('submit', async (e) => {
  ...
  await ocupado($('btn-facturacion'), 'Guardando…', async () => {
    ...
    if (error) {
      avisar('We couldn\'t save your details. Try again.');
      return;
    }
...
const pedirEnlaces = async () => {
  ...
  if (error) {
    ...
    avisar('We can\'t complete the purchase right now. Write to us at soporte@micarga.es or on WhatsApp at +34 744 716 449 and we\'ll activate it for you.');
    mostrarPaso('paso-correo');
    return;
  }

  if (!enlaceUsable(data?.enlaceMensual) || !enlaceUsable(data?.enlaceAnual)) {
    avisar('We can\'t complete the purchase right now. Write to us at soporte@micarga.es and we\'ll activate it for you.');
    mostrarPaso('paso-correo');
    return;
  }
...
const contratar = async (periodo, boton) => {
  ...
  if (error) {
    ...
    avisar(cuerpo?.error || 'We can\'t complete the purchase right now. Write to us at soporte@micarga.es or on WhatsApp at +34 744 716 449 and we\'ll activate it for you.');
    return;
  }
